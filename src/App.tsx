...
export function App() {
  ...
  const isUrlMobileScanner = searchParams.get('mobile_scanner') === '1' || searchParams.get('phone_scanner') === '1';
  const stationParam = searchParams.get('station') || '';
  const [isMobileScannerMode, setIsMobileScannerMode] = useState(isUrlMobileScanner);
    ...
    return (
      <MobileScannerView
        ...
        onExit={() => {
          ...
          if (typeof window !== 'undefined' && window.location.search.includes('mobile_scanner')) {
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col md:flex-row text-slate-800">
      {/* Mobile Top App Bar with Hamburger & Phone Scanner Trigger */}
      <div className="md:hidden bg-white border-b border-slate-200 px-4 py-3 flex items-center justify-between sticky top-0 z-40 shadow-xs">
        <div className="flex items-center gap-2.5">
          <button
            ...
            className="p-2 rounded-lg text-slate-600 hover:bg-slate-100 hover:text-slate-900 transition focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer"
        ...
        <div className="flex items-center gap-1.5">
          {/* Quick Launch Phone Scanner on Mobile */}
          <button
            onClick={() => setIsMobileScannerMode(true)}
            className="px-2.5 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-semibold shadow-xs flex items-center gap-1 cursor-pointer"
            title="Use this phone as barcode scanner for your packing workstation"
          >
        ...
        <nav id="left-sidebar-navigation" className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
          ...
          <div className="pt-3 border-t border-slate-100">
            <button
              ...
              className="w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-semibold bg-blue-50 text-blue-800 border border-blue-200 hover:bg-blue-100 transition cursor-pointer shadow-2xs"
              title="Switch this device into a high-speed wireless barcode scanner gun"
            >
              <div className="flex items-center gap-2.5">
                <Smartphone className="w-4 h-4 text-blue-600" />
                <span>Phone Barcode Gun</span>
              </div>
